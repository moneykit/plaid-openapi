...
class WalletTransactionListRequest(BaseModel):
    ...
    cursor: Optional[str] = Field(default=None, description="A value representing the latest transaction to be included in the response. Set this from `next_cursor` received in the previous `/wallet/transaction/list` request. If provided, the response will only contain that transaction and transactions created before it. If omitted, the response will contain transactions starting from the most recent, and in descending order by the `created_at` time.")
...
WalletTransactionListRequest.update_forward_refs()
